...
class GameData:
    ...
    def format(self):
        def summary():
            ...
            # write to json file 
            info = {}
            info['GameCount'] = self.count
            info["OurTeaminfo"]=[]
            info['OurTeaminfo'].append({
                'Goal': self.left_goals,
                'Point': self.left_points,
                'AvgGoal': self.avg_left_goals,
                'AvgPoint': self.avg_left_points,
                'WinCount' : self.win_count,
                'DrawCount' : self.draw_count,
                'DrawCount' : self.draw_count,
                'LostCount' : self.lost_count,
                'WinRate': self.win_rate*100,
                'ExpWinrate' : self.expected_win_rate*100
            })
            info['OppTeaminfo']=[]
            info['OppTeaminfo'].append({
                'Goal': self.right_goals,
                'Point': self.right_points,
                'AvgGoal': self.avg_right_goals,
                'AvgPoint': self.avg_right_points,
            })
            json_object = json.dumps(info, indent = 4) 
            
            with open("/var/lib/gitlab-runner/builds/c_ugVwfZ/0/kn2c-cicd/ci-developement/GameData.json", "w") as outfile: 
...
parser.add_option("-C", "--console", action="store_true", dest="console", default=False, help="print with color to console [default]")
parser.add_option("-N", "--no-color", action="store_true", dest="no_color", default=True, help="print without color to console")

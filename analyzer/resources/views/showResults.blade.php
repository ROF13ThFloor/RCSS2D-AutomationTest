...
@foreach ($All_GameData as $key => $value)
        @php
            $GameWDLInfo[$key]['DCount']=$value['OurTeaminfo'][0]['DrawCount'];
            $GameWDLInfo[$key]['WCount']=$value['OurTeaminfo'][0]['WinCount'];
            $GameWDLInfo[$key]['LCount']=$value['OurTeaminfo'][0]['LostCount'];
            $expWinrate[$key]= $value['OurTeaminfo'][0]['ExpWinrate'];
            $winRate[$key]=$value['OurTeaminfo'][0]['WinRate'];
            $OppAvgGoals[$key]=$value['OppTeaminfo'][0]['AvgGoal'];
            $oppGoals[$key]=$value['OppTeaminfo'][0]['Goal'];
            $avggoalzade[$key]=$value['OurTeaminfo'][0]['AvgGoal'];
            $goalzade[$key]=$value['OurTeaminfo'][0]['Goal'];
        @endphp
@endforeach
...
<head>
    ...
    <script type="text/javascript">var GameWLDData =<?php echo json_encode($GameWDLInfo); ?>;</script>
            ...
            <div class="row">
                @foreach ($All_GameData as $key => $value)
                    ...
                    <div class="col card card-1">
                        ...
                        <li class="repoinfo">Branch Name : {{ $Branch }}</li>
                        <li class="repoinfo">Test Type :{{ $test_Type }}</li>
                        <li class="repoinfo">Game Count : {{ $GameCount }}</li>
                ...
                @endforeach
                    ...
                    <div class="row">
                    @foreach ($GameWDLInfo as $key => $value)

                <div id={{$key}}
                               class="col" style="width: 900px; height: 500px;"></div>
                @endforeach
                    </div>
